"""
app.py — Universal Cutsheet to 3D CAD platform.
...
st.set_page_config(page_title="Cutsheet to 3D CAD", page_icon="🔧", layout="wide")

st.title("🔧 Cutsheet to 3D CAD  •  MVP v3")
st.caption("Upload cutsheet → extract specs → generate 3D → download STL, IFC, or DXF.")
...
if eq_type == "unknown" or not items:
    st.error("❌ Could not recognize this cutsheet format. Supported: Bell & Gossett PL pumps, Trane CLCA AHUs.")
    st.stop()
...
def _pump_download_row(mesh, model_name, row, A, B, C, D, E, flange_in):
    ...
    st.caption("💡 For native .dwg: open the .dxf in AutoCAD → File → Save As → .dwg (3 seconds).")


def _ahu_download_row(mesh, model_name, row, casing, W, H, total_L, section_triples):
    st.subheader("Download")
    c1, c2, c3 = st.columns(3)
    with c1:
        stl = mesh.export(file_type="stl")
...
if eq_type == "pump":
    ...
    with c1:
        ...
        D = st.number_input("D - Motor Dia (mm)",  value=float(row["D_mm"] or 105))
        E = st.number_input("E - Flange Dia (mm)", value=float(row["E_mm"] or 110))
    ...
    _pump_download_row(mesh, model_name, row, A, B, C, D, E, flange_in)
    ...
    with c1:
        ...
        section_triples = [
            (k, lbl, get_section_length(k, model_name))
            for (k, lbl) in selected_keys
        ]
    ...
    _ahu_download_row(mesh, model_name, row, casing, W, H, total_L, section_triples)


st.markdown("---")
st.caption(
    "Roadmap: STEP (pythonocc) · native .rfa and .dwg via Autodesk APS · "
    "MEP connectors in IFC · more equipment templates."
)
